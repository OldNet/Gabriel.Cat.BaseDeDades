...
{//de moment double i dateTime no els carrega bé de la BD mysql (access no s'ha probat) es pot utilitzar long en els dos casos en double s'hagafen el bytes i es converteix a long i datetime s'utlitzan els ticks
	...
	public abstract class ObjecteSql : IComparable<ObjecteSql>
	{
		private struct DadesSql
		{
			...
			public DadesSql(object dades, TipusDades tipus)
			{
				this.dades = dades;
				this.tipus = tipus;
        ...
        public static string EncryptString(string str,Key keyXifrat)
        {
            if (keyXifrat == null)
                throw new ArgumentNullException("keyXifrat");
            if (str == null)
		...
		public static double EncryptNumero(double numero, Key keyXifrat)
		{
			if (keyXifrat == null)
				throw new ArgumentNullException("keyXifrat");
        ...
        public static string DoubleToString(TipusBaseDeDades tipusBD, int parteEntera = 38, int parteDecimal = 15)
		{
			...
			switch (tipusBD) {
				...
				case TipusBaseDeDades.MySql:
					doubleString = " float(" + (parteEntera + parteDecimal) + "," + parteDecimal + ") ";
					break;
				...
				case TipusBaseDeDades.MySql:
					switch (type.AssemblyQualifiedName) {
						case Serializar.LONGASSEMBLYNAME:
							strAutoIncrement = " bigint AUTO_INCREMENT";
							break;
						case Serializar.INTASSEMBLYNAME:
							strAutoIncrement = " int AUTO_INCREMENT";
							break;
